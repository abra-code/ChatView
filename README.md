...
Images in message Markdown are fetched from the network as soon as a message renders. That is also a way for a model or an agent to send data out: an image URL can carry anything, and the fetch delivers it. `"remoteImages": "on-click"` in the configuration shows a placeholder naming the image's host until the reader clicks it, and `"never"` never fetches (`"automatic"` is the default; a present but unrecognized value counts as `"on-click"`, so a typo never turns the protection off). It covers every Markdown surface of the transcript: messages, thoughts, tool-call details and captions (RichText's `remoteImages`). `data:` images always show. The Android port does not read the key yet.

### Find

The transcript find bar opens with Cmd-F (`showFindBar: false` in the configuration removes the bar and the shortcut). Matches inside Markdown are painted by RichText's draw-only highlight layer, so lighting up a long conversation re-lays-out nothing; next / previous walk the hits in transcript order across messages, opening a folded thought or tool card when the hit is inside it. The bar's menu widens the scope to thoughts and tool calls and toggles case, whole-word, diacritic and regular-expression matching (a pattern is matched field by field, so `^` and `$` bound one message's lines; a pattern that does not compile reads "Invalid expression").
